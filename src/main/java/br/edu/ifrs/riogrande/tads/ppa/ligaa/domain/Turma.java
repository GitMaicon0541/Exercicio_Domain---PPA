...
public class Turma {
    ...
    public void turmaEstaAberta() {
        if (this.isFechada()) {
            throw new DomainException("Turma " + this.codigo + " Turma fechada");
        }
    }
    ...
    public void haVagas(Historico historico) {
        ...
        if (qtdAlunosTurma >= qtdVagas) { // não
            ...
            if (!reprovadoAnteriormente) {
                throw new DomainException("Não existe vagas na turma " + this.getCodigo());
            }
        }
    }

    public void estaMatriculado(Aluno aluno) {
        if (this.getMatriculas().stream().anyMatch(m -> m.getAluno().equals(aluno))) {
        throw new DomainException("Aluno " + aluno.getCpf() + "Aluno  matriculado na turma " + this.codigo);
        }
    }
